...
class APHOrderSinglePage
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_manageHostingOrder', array($this, 'manageHostingOrder'));
    }

    public function addMenu()
    {
        add_submenu_page('apphosting_settings', esc_html__('Orders', 'apphost'), esc_html__('-', 'apphost'), 'manage_options', 'app_each_order', array($this, 'display'));
    }
    ...
    public function display()
    {
        ...
        $order = $inobj->getById($id_order);
        if(!is_object($order) && !isset($order->id_order)) {
            ob_clean();
            APHTools::redirect(admin_url().'?page=app_customer_orders');
            exit;
        }
...
<div class="modal fade" id="ap_create_new_order_modal" tabindex="-1" aria-hidden="true">
        ...
        ?>
                                ...
                                <!--end::Input group--></div>
                                    ...
                                    <select class="form-control form-control-solid" placeholder="Plan" name="id_plan">
                                        ...
                                        <option value="<?php echo $p->id_plan; ?>"> <?php echo $p->name.' | '.$u->plan_type. ' | '.$p->app_type; ?></option>
                                    <?php  endforeach; endif;
        ?>
                                ...
                                <!--end::Input group--></div>
                                 ...
                                 <div class="row">
                                    ...
                                    <div class="col-sm-3">
                                        <div class="mb-5">
                                    <!--begin::Label-->
                                    <label class="required fs-5 fw-semibold mb-2">Registration Years</label>
                                    <!--end::Label-->
                                    <!--begin::Input-->
    ...
    public function manageHostingOrder()
    {
        ...
        } elseif($request_token =='APKJNRJWLGGDFTUETTDIWRXGGGHEELOFJWIVMTLLFMPRBTQOCXXXUQUFFQGJWHZM') {
            $user_id = (int) APHTools::getValue('user_id');
            $id_plan = (int) APHTools::getValue('id_plan');
            $domain_name = APHTools::getValue('domain_name');
            $domain_action = APHTools::getValue('domain_action');
